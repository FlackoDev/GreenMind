...
public class QuizDao {
    ...
    // ---- colonne (così eviti stringhe sparse ovunque) ----
    ...
    // versione utile per transazioni (ti passo db dall’esterno)
    public long upsert(SQLiteDatabase db, Quiz quiz) {
        ContentValues cv = toContentValues(quiz);

        // Se esiste già lo stesso id, lo sostituisce (update “semplice”)
    ...
    // -----------------------------
    // SELECT ALL (opzionale: ordina per title)
    // -----------------------------
